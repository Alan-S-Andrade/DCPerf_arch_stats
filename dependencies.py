...
def main():
    ...
    #block len
    plot_cdfs_block_len(block_dict)
    write_percentile_table("block_size_cdf", block_dict, output_dir)

    # taken not taken run
    plot_cdfs_taken_not_taken(taken_dict, "CDF of Consecutive Taken Conditional Branch Run Lengths", OUT_DIR / "taken_run_length_cdf.png")
    plot_cdfs_taken_not_taken(not_taken_dict, "CDF of Consecutive Not-Taken Conditional Branch Run Lengths", OUT_DIR / "not_taken_run_length_cdf.png")
    write_percentile_table("taken_run_cdf", taken_dict, output_dir)
    write_percentile_table("not_taken_run_cdf", not_taken_dict, output_dir)
    ...
    # --- Plots across traces ---
    if jump_dict:
        plot_jump_cdfs(jump_dict, output_dir, args.kernelSpace)
        write_percentile_table("jump_cdf", jump_dict, output_dir)
    else:
        print("No jumps found to plot.")

    if pairs_by_trace:
        write_pairs_csv(pairs_by_trace, output_dir)
        #plot_jump_vs_blocklen_scatter(pairs_by_trace, output_dir, args.kernelSpace)
    else:
        print("No (block_len, jump_size) pairs to plot.")

    # --- RAW CDFs across traces ---
    if raw_dict:
        plot_raw_cdfs(raw_dict, output_dir)
        if args.rawCSV:
    ...
    if args.familyApp:
        ...
        if single_app_instrs and single_app_blocks:
            ...
            plot_family_cdfs_single_app(args.familyApp, family_lists, output_dir)
            plot_family_stacked_bar_single_app(args.familyApp, family_lists, output_dir)
