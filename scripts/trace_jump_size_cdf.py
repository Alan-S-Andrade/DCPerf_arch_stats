...
def group_blocks(instrs):
    """Returns list of basic blocks based on starting and end virtual addresses"""
    if not instrs:
        return []
    blocks = []
    curr = instrs[0]
    ...
    for inst in instrs[1:]:
        ...
        if (inst.obj == curr.obj) and (inst.sym == curr.sym) and (curr.off is not None and inst.off is not None):
            # heuristic for determining block end: delta of current and next instruction in the stream
            # is 0 < Delta_y <= 8
            if (inst.va - curr.va > 0 and inst.va - curr.va <= 8):
                sequential = True
        if sequential:
            end_va = inst.va
        else:
...
def plot_cdfs(jump_dict):
    ...
    plt.xlabel("Absolute Jump Size (VA difference, log scale)")
    plt.ylabel("CDF")
    plt.title("CDF of Jump/Branch Target Sizes Across Applications (log-x) zoom")
    plt.xscale("log")
    plt.grid(True, which="both", linestyle="--", alpha=0.6)
    plt.legend()
    plt.tight_layout()
    logx_path = OUT_DIR / "jump_cdf_abs_logx_80.png"
    plt.ylim(0.8, 1.0)
    plt.savefig(logx_path, dpi=150)
    plt.close()
    print(f"Wrote {logx_path}")
...
def main():
    ...
    for trace_path in args.traces:
        ...
        print(f"{path.name}: grouped {len(instrs)} instructions: into {len(blocks)} blocks with {len(jumps)} jumps")
